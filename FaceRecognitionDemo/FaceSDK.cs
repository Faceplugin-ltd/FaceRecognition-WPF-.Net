...
namespace FaceSDK
{
    ...
    public class FaceEngineClass
    {
        ...
        public int Extract(byte[] rgbData, int width, int height, int stride, float[] feature)
        {
            ResultBox[] resultBoxes = new ResultBox[10];
            int maxFaceCount = 10;
            DetectFace(rgbData, width, height, stride, resultBoxes, maxFaceCount);
            ...
            try
            {
                int ret = Faceplugin_extract(imgPtr1, width, height, stride, resultBoxes[0], feature);
                return ret;
            }
            finally
            {
